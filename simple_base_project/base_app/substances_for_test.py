water = \
{'charge': 0,
 'elements_dict': {'H': 2, 'O': 1},
 'elements_list': ["H", "O"],
 'inchi': 'InChI=1S/H2O/h1H2',
 'inchikey': 'XLYOFNOQVPJJNP-UHFFFAOYSA-N',
 'molar_weight': 18.01528,
 'molecular_formula': "H2O",
 'smiles': 'O'}
nacl = \
{'charge': 0,
 'elements_dict': {'Cl':1, "Na": 1},
 'elements_list': ['Na', 'Cl'],
 'inchi': 'InChI=1S/ClH.Na/h1H;/q;+1/p-1',
 'inchikey': 'FAPWRFPIFSIZLT-UHFFFAOYSA-M',
 'molar_weight': 58.442769,
 'molecular_formula': 'NaCl',
 'smiles': "[Na+].[Cl-]"}
iproh = \
{'charge': 0,
 'elements_dict': {'C': 3, 'H': 8, 'O': 1},
 'elements_list': ["C", "H", "O"],
 'inchi': 'InChI=1S/C3H8O/c1-3(2)4/h3-4H,1-2H3',
 'inchikey': 'KFZMGEQAYNKOFK-UHFFFAOYSA-N',
 'molar_weight': 60.09502,
 'molecular_formula': 'C3H8O',
 'smiles': 'CC(C)O'}
stigm = \
{'charge': 0,
 'elements_dict': {'C': 29, 'H': 52},
 'elements_list': ['H', 'C'],
 'inchi': 'InChI=1S/C29H52/c1-7-22(20(2)3)12-11-21(4)25-15-16-26-24-14-13-23-10-8-9-18-28(23,5)27(24)17-19-29(25,26)6/h20-27H,7-19H2,1-6H3/t21-,22-,23?,24+,25-,26+,27+,28+,29-/m1/s1',
 'inchikey': 'GKBHKNPLNHLYHT-LWQAOISPSA-N',
 'molar_weight': 400.72318,
 'molecular_formula': 'C29H52',
 'smiles': 'CC(C)[C@H](CC)CC[C@@H](C)[C@H]1CC[C@H]2[C@@H]3CCC4CCCC[C@]4(C)[C@H]3CC[C@]12C'}
tio2 = \
{'charge': 0,
 'elements_dict': {"O":2, "Ti": 1},
 'elements_list': ['O', 'Ti'],
 'inchi': 'InChI=1S/2O.Ti',
 'inchikey': 'GWEVSGVZZGPLCZ-UHFFFAOYSA-N',
 'molar_weight': 79.8658,
 'molecular_formula': 'TiO2',
 'smiles': 'O=[Ti]=O'}
cuso4aq = \
{'charge': 0,
 'elements_dict': {'H': 10, 'Cu': 1, 'O':9, 'S': 1},
 'elements_list': ['H', 'Cu', 'O', 'S'],
 'inchi': 'InChI=1S/Cu.H2O4S.5H2O/c;1-5(2,3)4;;;;;/h;(H2,1,2,3,4);5*1H2/q+2;;;;;;/p-2',
 'inchikey': 'JZCCFEFSEZPSOG-UHFFFAOYSA-L',
 'molar_weight': 249.685,
 'molecular_formula': 'H10CuO9S',
 'smiles': 'O.O.O.O.[Cu+2].O.[O-]S([O-])(=O)=O'}
ti_ipro4 = \
{'charge': 0,
 'elements_dict': {'C': 12, 'H': 28, 'O':4, 'Ti': 1},
 'elements_list': ['C', 'H', 'O', "Ti"],
 'inchi': 'InChI=1S/4C3H7O.Ti/c4*1-3(2)4;/h4*3H,1-2H3;/q4*-1;+4',
 'inchikey': 'VXUYXOFXAQZZMF-UHFFFAOYSA-N',
 'molar_weight': 284.21532,
 'molecular_formula': 'C12H28O4Ti',
 'smiles': 'CC(C)O[Ti](OC(C)C)(OC(C)C)OC(C)C'}
so4 = \
{'charge': -2,
 'elements_dict': {'O': 4, 'S': 1},
 'elements_list': ["O", "S"],
 'inchi': 'InChI=1S/H2O4S/c1-5(2,3)4/h(H2,1,2,3,4)/p-2',
 'inchikey': 'QAOWNCQODCNURD-UHFFFAOYSA-L',
 'molar_weight': 96.06396972,
 'molecular_formula': 'O4S-2',
 'smiles': '[O-]S([O-])(=O)=O'}
