...
LOADER_PATH = ("D:\\playground\\chem_sandbox\\simple_base"
               "\\simple_base_project\\chemicals\\tests\\for_loader.json")


AMPERSANDTAGS_USERS = {
    "lorem": "LarysaHrybalova",
    "ipsum": "LarysaHrybalova",
    "dolor": "DarthVader",
    "sit": "DarthVader",
    "amet": "BillieEilish",
    "consectetur": "BillieEilish",
}

HASHTAGS = ("adipiscing", "elit", "sed")
...
def generate_hazard_str():
    number_of_hazards = random.randint(0, 3)
    hazards_indices = []
    hazards_str = ""
    while len(hazards_indices) < number_of_hazards:
        new_number = random.randint(0, 8)
        if not new_number in hazards_indices:
            hazards_indices.append(new_number)
    hazards_indices.sort()
    hazards_str = ", ".join(
        list(
            map(
                lambda x: HAZARD_TYPES[x],
                hazards_indices
            )
        )
    )
...
def generate_hashtags():
    ...
    for i in range(3):
        ...
        if j in (9, 10):
            hashtags.append(random.choice(HASHTAGS))
    return set(hashtags)


def generate_ampersandtags():
    ampersandtags = dict()
    items = list(AMPERSANDTAGS_USERS.items())
    for i in range(3):
        j = random.randint(0, 10)
        if j == 0:
            tag, owner = random.choice(items)
            ampersandtags[tag] = owner
    return ampersandtags
...
def load_storages():
    ...
    a = StoragePlace.create("Лаб. 416", root)
    a.save()
    b = StoragePlace.create("Шкаф 1", a)
    b.save()
    c = StoragePlace.create("Полка 1", b)
    c.save()
    d = StoragePlace.create("Полка 2", b)
    d.save()
    e = StoragePlace.create("Полка 3", b)
    e.save()
    f = StoragePlace.create("Шкаф 2", a)
    f.save()
    g = StoragePlace.create("Верхняя полка", f)
    g.save()
    h = StoragePlace.create("Средняя полка", f)
    h.save()
    i = StoragePlace.create("Нижняя полка", f)
    i.save()
    j = StoragePlace.create("Холодильник", a)
    j.save()
    k = StoragePlace.create("Верхняя полка", j)
    k.save()
    l = StoragePlace.create("Нижняя полка", j)
    l.save()
    m = StoragePlace.create("Дверь", j)
    m.save()
    n = StoragePlace.create("Лаб. 401", root)
    n.save()
    o = StoragePlace.create("Шкаф", n)
    o.save()
    p = StoragePlace.create("Коробка 1", o)
    p.save()
    q = StoragePlace.create("Коробка 2", o)
    q.save()
    r = StoragePlace.create("Верхняя полка", o)
    r.save()
    s = StoragePlace.create("Холодильник", n)
    s.save()
    t = StoragePlace.create("Верхняя полка", s)
    t.save()
    u = StoragePlace.create("Нижняя полка", s)
    u.save()
    v = StoragePlace.create("Шуфлядка", s)
    v.save()
    w = StoragePlace.create("Сейф", n)
    w.save()
    x = StoragePlace.create("Лаб. 425", root)
    x.save()
    y = StoragePlace.create("Шкаф 1", x)
    y.save()
    z = StoragePlace.create("Полка 1", y)
    z.save()
    aa = StoragePlace.create("Полка 2", y)
    aa.save()
    ab = StoragePlace.create("Полка 3", y)
    ab.save()
    ac = StoragePlace.create("Холодильник 1", x)
    ac.save()
    ad = StoragePlace.create("Полка 1", ac)
    ad.save()
    ae = StoragePlace.create("Полка 2", ac)
    ae.save()
    af = StoragePlace.create("Холодильник 2", x)
    af.save()
    ag = StoragePlace.create("Верхняя полка", af)
    ag.save()
    ah = StoragePlace.create("Средняя полка", af)
    ah.save()
    ai = StoragePlace.create("Нижняя полка", af)
    ai.save()
...
def initialize_storages():
    terminal_storages = []
    for storage in StoragePlace.objects.all():
        if not storage.has_children:
            terminal_storages.append(storage)
    for storage in terminal_storages:
...
def load_chemicals():
    ...
    all_storage_places = StoragePlace.objects.all()
    for storage_place in all_storage_places:
        storage_places[storage_place.path_str] = storage_place
    all_units = QuantityUnit.objects.all()
    for quantity_unit in all_units:
        units[quantity_unit.unit_symbol] = quantity_unit

    # paste Dump file name/location below:
    with open(LOADER_PATH, "rt", encoding="utf-8") as file:
        raw_json = file.read()
    all_records = json.loads(raw_json)
    for i, record in enumerate(all_records):
        mol_block = record["mol_block"]
        lazy_mol_instance = LazyMol(mol_block, form="mol_block")
        elements_dict = lazy_mol_instance.elements_dict()
        path_dict = lazy_mol_instance.path_dict()
        ring_dict = lazy_mol_instance.ring_dict()
        molar_weight = lazy_mol_instance.molar_weight
        
        del record["elements_dict"]
        record["quantity"] = D(record["quantity"])
        record["molar_mass"] = molar_weight
        ...
        record["hazard_pictograms"] = generate_hazard_str()
        new_chemical = Chemical.create(record,
                                       elements_dict,
                                       path_dict,
                                       ring_dict)
        new_chemical.save()
        ...
        new_chemical.initialize(barcode=barcode)
        billie = profiles["BillieEilish"]
        for tag in generate_hashtags():
            Hashtag.bind(chemical=new_chemical, tag=tag, user=billie)
        for tag, username in generate_ampersandtags().items():
            Ampersandtag.bind(tag=tag,
                              user=profiles[username],
                              chemical=new_chemical)
        if i % 100 == 0:
            print(i, "chemicals loaded")
